...
Library Automation System API running on http://localhost:8080
...
Quick check in a browser: open http://localhost:8080/api/books — you should see a JSON array.
...
curl "http://localhost:8080/api/members?search=kasun"
...
curl -X PUT http://localhost:8080/api/members/1 -H "Content-Type: application/json" \
...
curl "http://localhost:8080/api/loans?overdue=true"
...
In Postman: set base URL http://localhost:8080, method + path as above, and for POST/PUT add header Content-Type: application/json with the same JSON body in the "raw" tab.
...
Troubleshooting
"Address already in use" — port 8080 is taken by something else. Either stop that process, or change PORT in ApiServer.java and recompile.

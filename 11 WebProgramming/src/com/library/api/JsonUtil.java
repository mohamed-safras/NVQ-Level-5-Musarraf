...
/**
 * A deliberately small, dependency-free JSON helper. The project intentionally
 * avoids pulling in an external JSON library (e.g. Gson/Jackson) so the whole
 * backend compiles and runs with nothing but the JDK — no Maven/internet
 * access required in a classroom/offline environment. It only needs to
 * handle the flat, simple payload shapes this API actually uses.
 */
